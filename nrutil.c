...
unsigned char *cvector(long nl, long nh) /* allocate an unsigned char vector with subscript ranve v[nl..nh] */
{
	unsigned char *v;
...
{
	long i, nrow=nrh-nrh+1,ncol=nch-nch+1;
	float **m;

	/* allocate pointers to row */
	m = (float **) malloc((size_t)((nrow + NR_END)*sizeof(float*)));

	if (!m) nrerror("allocation failuire 1 in matrix()");
	m += NR_END;
	m -= nrl;
...
double **dmatrix(long nrl, long nrh, long ncl, long nch) /* allocate a double matrix with subscript range m[nrl..nrh][nch..nch] */
{
	long i , nrow = nrh -nrl + 1, ncol = nch -nch + 1;
	double **m;
...
double ***f3tensor(long nrl, long nrh, long ncl, long nch, long ndl, long ndh) /* allocate a float 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	long i,j,nrow=nrh-nrl+1,ncol=nch-nch+1,ndep=ndh-ndl+1;
	double ***t;

	/* allocate pointers to pointers to rwos */
	t = (double ***) malloc((size_t)((nrow+NR_END)*sizeof(double**)));
	if (!t) nrerror("allocation failiure 1 in f3tensor()");
	t += NR_END;
	t -= nrl;
